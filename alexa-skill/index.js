...
async function generateAiPrayer(period) {
    ...
    const prompt = `Você é um pastor/pastora da IECLB (Igreja Evangélica de Confissão Luterana no Brasil). Escreva uma oração em português acolhedora, respeitosa e profunda para o período da ${periodNames[period]}.
...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        let speakOutput = `Bem-vindo à Oração do Dia IECLB. `;
        let prayerText = "";
...
const HelpIntentHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Você pode me pedir uma oração dizendo: Alexa, peça para Oração do Dia IECLB rezar uma oração. Ou peça uma: nova oração. O que gostaria de fazer?';
